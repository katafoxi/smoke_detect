...
# overlay.video_sink \
# \
# filesrc location=./streams/${SRT_FILE} !\
...
mkfifo /tmp/subtitles.fifo 2>/dev/null || true

# Запускаем оба процесса параллельно
python3 gpsd_row_to_subtitle.py &
    # do-timestamp=true \
gst-launch-1.0 -e \
textoverlay name=overlay wait-text=true font-desc="Sans, 62" !\
autovideosink \
...
overlay.video_sink \
\
filesrc location=/tmp/subtitles.fifo !\
queue !\
subparse !\
overlay.text_sink &

# Ждем завершения (нажмите Ctrl+C чтобы прервать)
wait

# Удаляем FIFO при завершении
rm /tmp/subtitles.fifo
...
# gst-launch-1.0  -v \
# filesrc location=/dev/ttyACM0 blocksize=1 ! queue !\
# text/x-raw,format=utf8 ! intersubsink

# Этот pipeline подключается к GPSD на localhost порт 2947 (стандартный порт GPSD) и выводит сырые данные в stdout.

#OK
# gst-launch-1.0 tcpclientsrc host=localhost port=2947 num-buffers=-1 ! queue ! fdsink 

# ./gpsd_row_to_subtitle.py && 
# gst-launch-1.0 filesrc location=/tmp/subtitles.fifo ! queue ! subparse ! queue ! fdsink 


# gst-launch-1.0 giosrc uri="unix:///var/run/gpsd.sock" ! fdsink
...
# text/x-raw,format=utf8 !  overlay.text_sink
